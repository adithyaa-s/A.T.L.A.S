...
from .tools.calender.calender_tools import get_current_time
...
# Configure paths for MCP servers
CALENDAR_MCP_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "../google-calendar-mcp"))
...
calendar_agent = Agent(
    ...
    You have access to these tools via the Google Calendar MCP server:
    - `list_events`: Show events from your calendar for a specific time period
    - `create_event`: Add a new event to your calendar 
    - `update_event`: Edit an existing event (change title or reschedule)
    - `delete_event`: Remove an event from your calendar
    - `find_free_slots`: Find available free time slots in your calendar
    ...
    tools=[
        MCPToolset(
            connection_params=StdioConnectionParams(
                server_params=StdioServerParameters(
                    command='node',
                    args=[os.path.join(CALENDAR_MCP_PATH, 'build/index.js')],
                ),
                env={
                    'GOOGLE_OAUTH_CREDENTIALS': os.path.join(CALENDAR_MCP_PATH, 'gcp-oauth.keys.json')
                }
            ),
        )
    ]
...
memory_agent = Agent(
    ...
    Remember: You help Atlas remember what matters to the user so future interactions are smarter.""",
...
root_agent = Agent(
    ...
    - Calendar agent: Handles all calendar operations via Google Calendar MCP (list events, create events, update events, delete events, find free time)
